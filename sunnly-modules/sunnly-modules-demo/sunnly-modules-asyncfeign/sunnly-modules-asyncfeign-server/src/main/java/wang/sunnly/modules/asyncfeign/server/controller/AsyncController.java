...
public class AsyncController {
    ...
    @GetMapping("future")
    public ObjectResult<List<String>> future() throws ExecutionException, InterruptedException {
        ...
        ObjectResult<List<String>> listObjectResult = new ObjectResult<>(returnValueManage.get());
        log.info("=====done ...{}, {}", Thread.currentThread().getName(), System.currentTimeMillis() - begin);
        return listObjectResult;
    }

    @GetMapping("future1")
    public ObjectResult<String> future1(){
        List<Future<String>> futures = new ArrayList<>();
        futures.add(asynFeignService.send(1));
        futures.add(asynFeignService.send(2));
        futures.add(asynFeignService.send(3));
        futures.add(asynFeignService.send(4));
        return new ObjectResult<String>().setData("success");
    }
}
